...
function stripDuplicateHeader(md: string): string {
  return md.replace(
    /^#\s+Structural Valuation and the Real-World Asset Divergence: A Deep-Dive Analysis of Centrifuge Protocol\s*\n+/m,
    ''
  );
}

function CentrifugeMarkdownBody({ markdown }: { markdown: string }) {
  return <ReactMarkdown remarkPlugins={[remarkGfm]}>{markdown}</ReactMarkdown>;
}

export function CentrifugeRWAStructuralValuation2026() {
  const [md, setMd] = useState('');
  const [err, setErr] = useState('');

  useEffect(() => {
    fetch('/content/insights/centrifuge-rwa-structural-valuation-2026.md', { cache: 'no-store' })
      .then((r) => r.text())
      .then((t) => {
      ...
      <article className="min-h-screen font-sans bg-gradient-to-b from-[#0A0A0A] via-[#050505] to-[#0A0A0A] relative">
        <div className="max-w-5xl mx-auto px-4 py-8 relative z-10">
          <nav className="flex items-center gap-2 text-sm text-gray-500 mb-8">
            ...
            <ChevronRight size={14} />
            <span className="text-gray-400">Centrifuge RWA Structural Valuation</span>
          </nav>
          ...
          <div className="mb-16">
            ...
            <p className="text-xl text-slate-400 mb-6 leading-relaxed">
              By Nexus One Research Desk • May 4, 2026 • 25 min read
            </p>
          </div>
          ...
          <ReportErrorBoundary>
            <div className="prose prose-invert prose-lg max-w-none text-slate-300 prose-headings:text-white prose-strong:text-white prose-a:text-blue-400 prose-a:break-all prose-li:marker:text-slate-400 prose-ul:list-disc prose-ol:list-decimal [&_del]:line-through [&_del]:text-slate-500 [&_s]:line-through [&_s]:text-slate-500 [&_table]:w-full [&_table]:table-fixed [&_table]:border-collapse [&_table]:border [&_table]:border-slate-800 [&_th]:border [&_th]:border-slate-800 [&_th]:px-4 [&_th]:py-3 [&_td]:border [&_td]:border-slate-800 [&_td]:px-4 [&_td]:py-3 [&_thead]:bg-slate-900/50 [&_tbody_tr:nth-child(even)]:bg-slate-900/30 [&_blockquote]:border-slate-600 [&_blockquote]:text-slate-400">
              <CentrifugeMarkdownBody markdown={body} />
            </div>
          </ReportErrorBoundary>
