...
> 코드베이스 실물 기준으로 검증됨 (2026-07-08, 동적 라우팅 도입 시점).

---

## 게시 구조 — ✨ 파일 2개면 끝 (2026-07-08부터)

`src/pages/InsightPage.tsx`(만능 페이지)와 `/insights/:slug` 동적 라우트가 도입되어,
**신규 리포트는 아래 2개만 추가하면 게시된다.**

| # | 파일 | 작업 |
|---|------|------|
| 1 | `public/content/insights/{slug}.md` | 마크다운 본문 업로드 |
| 2 | `src/data/reports.ts` | 카드 메타데이터 블록을 **배열 맨 위**에 추가 |

동작 원리: reports.ts의 `id`가 곧 주소(`/insights/{id}`)가 되고, InsightPage가
`id`와 같은 이름의 `.md`를 찾아 렌더링한다. 제목·저자·날짜·readTime·배지도
reports.ts 값을 그대로 쓴다. **따라서 md 파일명 = reports.ts의 id 가 반드시 같아야 함.**
